...
| **Despliegue activo** | **`@11`** — "ADR-080: recover dice si hay inscripcion para el curso" (21-sep-2026). ⚠️ **Este número envejece solo**: esta fila dijo `Versión 6` durante tres meses con producción en `@8`, `@9` y `@10`. **La fuente de verdad es `clasp list-deployments`**, y el Paso 6 de `verificar-backend.js` lo comprueba contra el endpoint vivo. |
...
| `AKfycbxxZBp6XpmdRzZS0BXO02WMq31K5FUU8-Mqzc2Sj0PcwB3cMcrhIqbHQA0naUQb5mgBWw` | **`@11`** | **PRODUCCIÓN** — es el ID que va en `PROD_DEPLOYMENT_URL` |
...
> 🚨 **Producción está fijada a una versión concreta (hoy `@11`, promovida el 21-sep-2026), no a `@HEAD`.** Un `clasp push` (o editar en el navegador y guardar) **no llega a los estudiantes**: hay que crear una versión nueva y reapuntar ese deployment. Es la causa más probable de "arreglé el backend y sigue fallando igual".
>
...
| 2026-09-21 (cierre 4) | **ADR-080.** `handleRecover()` recibía `params.course` y **lo tiraba**: buscaba por correo y devolvía la **primera** fila que encontrara, del curso que fuera. El motor entraba con ese registro **ajeno**, la persona hacía el curso y al final se escribía su certificado **pero nunca su inscripción**. ⚠️ **6 de los 7 huérfanos eran la línea Desarrollo Institucional entera**, que el panel mostraba con **0 adultos** teniendo sus seis cursos completados. | `recover` devuelve **`registeredInCourse`** para el curso preguntado (`null` si no se preguntó) y **prefiere la inscripción de ese curso** cuando existe; el alta la crea el **motor, por POST y solo ante un `false` explícito**. **Este GET sigue sin escribir**, a propósito — uno que escribiera dejaría crear inscripciones a quien sepa un correo. Nace `probar-recover.js` (8 comprobaciones, probada disparando), **32 cursos reconstruidos** y **453 pruebas en verde**. Promovido a **`@11`**. |
...
_Última actualización: 2026-09-21 (ADR-080, producción en `@11`)._
